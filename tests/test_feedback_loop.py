...
def test_human_feedback_loop_keeps_content():
    # Simulate user pressing ENTER twice (empty input, then break)
    with patch("builtins.input", side_effect=["", ""]):
        result = human_feedback_loop("original content")
        assert result == "original content"

def test_human_feedback_loop_returns_user_input():
    # Simulate user entering a line, then ENTER to break
    with patch("builtins.input", side_effect=["user edit", ""]):
        result = human_feedback_loop("original content")
